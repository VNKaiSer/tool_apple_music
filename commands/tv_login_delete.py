...
def update_mail_re_run(id):
    db_instance.update_data(table_name="mail", set_values={"isRunningLoginDelete": "N"}, condition="id = %s" % id)
def run():    
    ...
    if check_account_is_block(driver):
        db_instance.insert_mail_delete([gmail, password, "UnLock", "","none"])
        exception = True
        return
    ...
    if check_account_login_invalid_password(driver):
        print("Sai pass")
        db_instance.insert_mail_delete([gmail, password, "sai pass", "","none"])
        exception = True
        return
    ...
    if check_account_has_otp(driver):
        db_instance.insert_mail_delete([gmail, password, "2FA", "","none"])
        exception = True
        return
    ...
    try:
        ...
        db_instance.insert_mail_delete([gmail, password, "", country, "none"])
    except Exception as e: # Đã có thẻ
        ...
        db_instance.insert_mail_delete([gmail, password, "", country, "bin"])
